...
data.t= as.data.frame(cbind(id=c(1:ni), time=ttoevent, delta=delta, x.1=data.x$x.1, x.2=data.x$x.2,x.3=data.x$x.3,x.4=data.x$x.4)) 
...
b.ini=mvrnorm(ni,rep(0,2*J),Sigma.ini)
sigma.e2.ini=1
coxfit1 = coxph(Surv(ttoevent,delta) ~ Xt[,1] + Xt[,2] + Xt[,3]+Xt[,4]+b.ini[,1:6],model = TRUE)
...
## function
{
  ...
  h0.hist <- h0.old <- h0 <- c(0.1,0.5,1.25,3,10)
